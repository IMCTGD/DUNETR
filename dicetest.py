...
def inference(model, criterion, train_loader, valid_loader, device, save_img_path, is_infer_train=True):
    ...
    if is_infer_train:
        ...
        for batch in (train_loader):
            ...
            with torch.no_grad():
                ...
                dice_list_sub.append(1-loss.cpu().detach().numpy())
                print(f"\033[31mTrain-ID:{batch['image_index']}  || Dice: {1-loss} \033[0m")
        ...
        # # 保存结果
        # record_dice = {}
        # record_dice['id'] = ID_list['train']
        # result = np.array(dice_list_sub)
        # record_dice['dice'] = result[:]
    ...
    for batch in (valid_loader):
        ...
        print(f"\033[31mVal-ID:{batch['image_index']}  || Dice: {organ_Dice} \033[0m")
        # ## 保存文件
        # ID = batch['image_index']
        # affine = batch['affine']
        # img_size = batch['image_size']
        # os.makedirs(save_img_path, exist_ok=True)
        # batch_save(ID, affine, val_outputs, img_size, save_img_path)
    mean_dice = np.mean(dice_list_sub)
    print("\033[31mACC_Dice:%f  \033[0m" % (mean_dice))
...
def args_input():
    ...
    p.add_argument('--model', type=str, default='SwinDUNETR')   # DUNETR、UNETR、FCN_AG_scr、Unet++、SwinDUNETR
    p.add_argument('--fold', type=int, default=1)           ## 选择哪个数字作为测试集
    p.add_argument('--load_num', type=int, default=156)     ## 0 代表不加载参数,    1加载
...
if __name__ == '__main__':
    ...
    # train_infer_loader = DataLoader(train_set, 1, num_workers=num_workers, shuffle=False)
    # valid_infer_loader = DataLoader(valid_set, 1, num_workers=num_workers, shuffle=False)
    # #
    train_set = get_loader(train_path, train_path, ID_list['train'], input_size, args, False)
    ...
    # result = CD.calculate_dice(ID_list['valid'])
